...
import static io.doist.recyclerviewext.sticky_headers.ConcatAdapterUtils.offsetPositionOnAdapter;
...
public class StickyHeadersGridLayoutManager extends OpenGridLayoutManager {
    ...
    private void createStickyHeader(@NonNull RecyclerView.Recycler recycler, int position) {
        ...
        Pair<? extends RecyclerView.Adapter<?>, Integer> adapterPositionPair = offsetPositionOnAdapter(mAdapter, position);
        if (adapterPositionPair.first instanceof OnViewStickyListener) {
            ((OnViewStickyListener) adapterPositionPair.first).setupStickyHeaderView(stickyHeader);
        }
    ...
    private void scrapStickyHeader(@Nullable RecyclerView.Recycler recycler) {
        ...
        int stickyHeaderPosition = mStickyHeaderPosition;
        mStickyHeader = null;
        ...
        Pair<? extends RecyclerView.Adapter<?>, Integer> adapterPositionPair = offsetPositionOnAdapter(mAdapter, stickyHeaderPosition);
        if (adapterPositionPair.first instanceof OnViewStickyListener) {
            ((OnViewStickyListener) adapterPositionPair.first).teardownStickyHeaderView(stickyHeader);
        }
    ...
    private class HeaderPositionsAdapterDataObserver extends RecyclerView.AdapterDataObserver {
        ...
        public void onChanged() {
            ...
            for (int i = 0; i < itemCount; i++) {
                Pair<? extends RecyclerView.Adapter<?>, Integer> adapterPositionPair = offsetPositionOnAdapter(mAdapter, i);
